...
### 2. 调试端点残留生产代码
**问题：** `web/api/portfolio.py` 中 `/api/portfolio/price-test` 是调试 IB 价格获取时
加的临时端点，应在发布前移除，避免暴露内部实现。
**修复：** 删除 `portfolio.py` 第 99-130 行左右的 `price_test()` 函数和路由。

---

## 🟡 中优先级（影响策略质量）

### 3. 回测缺乏 Walk-Forward 验证
...
### 7. DataStore `_load()` 每个股票读 Parquet 两次
