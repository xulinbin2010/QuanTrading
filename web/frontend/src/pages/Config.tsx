...
function ConnCard({ group, params }: { group: string; params: ConnParam[] }) {
  return (
    <div className="bg-slate-800 rounded-lg border border-slate-700 p-4">
      <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">
        {group === 'MySQL' ? '🗄 MySQL 数据库' : '📡 IB Gateway'}
      </div>
...
export default function Config() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {/* 连接参数（只读，放最后） */}
      <div className="max-w-2xl">
        <div className="text-sm font-medium text-slate-300 mb-3">连接参数（只读）</div>
        <div className="space-y-4">
          {Object.entries(connByGroup).map(([group, params]) => (
            <ConnCard key={group} group={group} params={params} />
          ))}
        </div>
      </div>
    </div>
